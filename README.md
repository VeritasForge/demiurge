...
| **CLI Tools** | 1 | `~/.local/bin/`으로 배포되는 git 보조 도구 |
...
`bin/` 패키지가 `~/.local/bin/`으로 stow 배포되어, PATH 등록 후 어디서나 호출 가능합니다.

| Command | 설명 |
|---------|------|
| `git cleanup-worktrees [base-branch]` | git worktree 상태 분석 + interactive 정리 (`-h` 도움말, `-f` 강제 삭제) |

> `--help`는 git이 `man` 페이지로 가로채므로 `-h` 사용. fish 사용자는 `bootstrap.sh`가 `fish_add_path -U`로 PATH 자동 등록.
...
│   ├── skills/ (42)
│   ├── agents/ (21)
│   ├── rules/ (2)            # 전역 룰: skills.md, agents.md (paths frontmatter 지원)
...
├── .claude/                  # 프로젝트 로컬 (demiurge 한정)
│   ├── rules/stow-deployment.md  # demiurge 전용 stow 배포 운영 규칙
│   └── commands/wrap.md
