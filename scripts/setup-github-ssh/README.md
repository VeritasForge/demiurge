...
| `~/.gitconfig` | 기본(회사) 커밋 저자 + `~/lab`, `~/.config/nvim` 개인 전환 규칙 | 아니오 |
...
① SSH 별칭 → "어느 계정으로 push되나"        ② 폴더 위치 → "커밋에 어떤 이메일이 찍히나"
   ...
   git@github.com-personal:... → 개인 키          ~/lab, ~/.config/nvim 아래 → 개인 이메일 (자동)
```

`~/.gitconfig`의 `includeIf "gitdir:~/lab/"`, `includeIf "gitdir:~/.config/nvim/"` 규칙이, 작업 중인 저장소가 `~/lab` 또는 `~/.config/nvim` 아래일 때만 개인 설정을 덮어씁니다.

```
                  git 명령을 실행한 폴더는?
                          │
          ┌───────────────┴────────────────┐
   ~/lab, ~/.config/nvim 아래         그 외 전부
          │                                │
...
- **개인 작업은 반드시 `~/lab`(또는 `~/.config/nvim`) 아래 + `-personal` 별칭 두 가지를 함께** 지켜야 합니다. 하나만 지키면 커밋 이메일과 push 계정이 어긋납니다.
...
| 커밋 이메일이 계속 회사 것 | 저장소가 `~/lab`, `~/.config/nvim` 밖에 있음. `git config user.email`로 확인 후 폴더 위치 조정. |
