...
> #1~#8은 README 구조·내용 문제, #9~#11은 한국어 문장 명료성 문제다. 후자는 `--lang en`일 때 적용하지 않는다.
...
| 9 | 문장 성분 생략 | 한국어 문장에서 주어·목적어가 빠져 "무엇이 그런지" 모호함. 관형격 조사 `~의`가 한 문장에 3회 이상 연쇄 | 생략된 성분을 복원하고 `~의` 연쇄를 서술어로 풀어 쓴다. 예: "사본의 문구는 작업의 상황을" → "사본에 기재된 문구는 작업이 진행되는 상황을" | 영문 README |
| 10 | 명사구 종결 | 문장이 서술어 없이 명사구·연결어미로 끝남. 예: "설치 후 실행." "빠른 시작을 위한" | 서술어와 종결어미를 붙여 완성된 문장으로 끝맺는다. 헤더·표·목록 항목은 제외 | 영문 README |
| 11 | 비유적 어휘 대체 | 일반 명사·동사 자리에 비유 표현. 예: "분석의 흐름", "코드에 박는다", "요청을 받습니다" | 맥락에 맞는 한자어·일반 어휘로 교체. 예: "분석 방향", "코드에 명시한다", "요청을 확인했습니다". 해당 분야에서 관용으로 굳은 표현은 유지 | 영문 README |
