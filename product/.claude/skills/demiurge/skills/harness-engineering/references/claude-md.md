...
- 세션 시작 때 한 번 읽으므로 **도중의 편집·git pull은 반영되지 않는다**. 반영하려면
  `/clear`나 재시작. 작업(PR) 단위로 세션을 새로 시작하면 팀원이 올린 최신본이 자연히
  반영되고 쌓인 컨텍스트도 비워진다.
...
- 주의: `/compact` 이후 path-scoped rules가 자동 재주입되는지는 공식 문서에 명시가 없다
  (스킬에 대해서만 "재주입 안 됨"이 명시됨). compact를 건너서도 항상 있어야 할 규칙은
  루트 CLAUDE.md나 `paths` 없는 rules에 두는 것이 안전하다.
