...
3. 대상의 도메인/기술/도구 키워드 추출
4. **대상 파일 경로 식별 및 기록** (Phase 6에서 참조)
   - 문서 검증 모드: `$ARGUMENTS`에서 추출한 파일 경로를 절대 경로(또는 워크스페이스 상대 경로)로 정규화하여 기록
   - 리서치/아이디어 검증 모드: 원본 파일 없음 → `N/A` (Phase 6에서 `/organize` 생성 모드로 진행)
   - 코드 리뷰 모드: PR 식별자만 기록 → Phase 6 스킵 대상이므로 `/organize` 미사용
...
## 대상
- **작업 설명**: {설명}
- **모드**: {문서 검증 | 코드 리뷰 | 리서치/아이디어 검증}
- **대상 파일 경로**: {Phase 1-4에서 정규화한 경로 | N/A (리서치) | PR #번호 (코드 리뷰)}
...
수렴 조건 충족 시 main agent가 convergence-evaluator에 최종 리포트 작성을 요청한 후, **Phase 6로 진행** (단, 코드 리뷰 모드는 Phase 6를 스킵하고 종료).

---

## Phase 6: 검증 결과 반영 미리보기 (자동 실행, 원본 무변경)

수렴 완료 후 **자동으로 실행**됩니다 (별도 사용자 동의 없이 진행). 검증 결과(report.md)를 검증 대상에 반영한 모습을 **화면에 출력**합니다.

> `/rl-verify` 스킬 자체는 검증 대상 원본 파일을 **절대 수정하지 않습니다.** 화면 출력 결과를 보고 원본에 덮어쓸지 여부는 `/organize` Step M5에서 **사용자가 직접 판단**합니다.

### 6-1. 모드별 분기

| 모드 | Phase 6 동작 |
|------|------------|
| 문서 검증 | `/organize @{Phase 1에서 식별한 대상 파일}` 자동 호출 (병합 모드) |
| 리서치/아이디어 검증 | `/organize` 자동 호출 (생성 모드) — 신규 문서 화면 출력 |
| 코드 리뷰 | **Phase 6 스킵** — report.md만 유지하고 종료 |

### 6-2. 실행 흐름

1. **모드 확인** — 코드 리뷰 모드면 즉시 종료 (`/organize` 미호출)
2. **report.md 컨텍스트 확보**
   - 현재 컨텍스트에 convergence-evaluator가 갱신한 report.md 최신 내용이 있으면 그대로 사용
   - 컨텍스트에 없거나 최신 여부가 불확실하면 Read 도구로 `docs/demiurge/rl-verify/{slug}/report.md`를 다시 읽어 보강
   - **호출 직전 report.md 내용을 명시적으로 사용자에게 노출**하여 `/organize`가 "병합 소스"로 인식할 수 있도록 함 (단순히 Read만 하고 끝내지 마라)
3. **`/organize` 자동 호출** (사용자 추가 동의 단계 없음)

   호출 시 다음 한 줄 지시를 함께 전달한다:
   > "검증 결과 리포트(`docs/demiurge/rl-verify/{slug}/report.md`)를 병합/생성 소스로 사용하라. 일반 대화 내용이 아닌 이 리포트의 확정 발견사항을 기준으로 반영하라."

   - **문서 검증 모드**: `/organize @{Phase 1에서 식별한 대상 파일 경로}` — 병합 모드
   - **리서치 모드**: `/organize "{Phase 0에서 생성한 slug 또는 검증 주제 키워드}"` — 생성 모드 (인자로 주제 명시하여 `/organize`가 다른 최근 대화 주제로 빠지지 않도록 고정)

4. **이후 동작은 `/organize` 스킬에 위임**
   - 병합 모드: Step M4에서 병합 미리보기 화면 출력 → M5에서 사용자가 저장 여부 명시 판단
   - 생성 모드: Step C4에서 신규 문서 화면 출력 (파일 저장 없음)

### 6-3. 원본 보존 원칙

- `/rl-verify` 스킬 자체는 검증 대상 파일을 **직접 수정하지 않음**
- 원본 덮어쓰기는 `/organize` Step M5에서 **사용자가 명시적으로 승인**한 경우에만 발생
- 사용자가 거부하면 원본은 보존되고 Phase 6 종료
- 즉 본 스킬의 절대 규칙 2를 위반하지 않음 — 원본 수정 권한은 사용자에게만 있음

### 6-4. 하지 말 것

- Phase 6에서 사용자에게 "반영할까요?" 같은 추가 동의를 묻지 마라 — 동의는 `/organize` Step M5에 위임
- `/organize` 대신 직접 Write 도구로 대상 파일을 수정하지 마라
- 코드 리뷰 모드에서 억지로 `/organize`를 호출하지 마라 — 코드는 마크다운 병합 대상이 아님
...
2. 검증 대상 코드/문서를 **직접 수정하지 않는다** — Phase 6는 `/organize` 호출로 화면 출력까지만 수행하며, 원본 덮어쓰기는 `/organize` Step M5에서 사용자가 명시 동의한 경우에만 발생
3. 수렴하지 않았는데 종료하지 않는다
4. 추측으로 "수렴했다"고 판단하지 않는다 — 실제 비교 근거 필요
5. 사용자 동의 없이 수렴 루프를 시작하지 않는다
6. Phase 6는 수렴 완료 시 자동 실행한다 (코드 리뷰 모드는 예외 — 스킵)
