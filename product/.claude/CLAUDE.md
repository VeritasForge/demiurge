...
9. Step by step으로 sequential thinking을 할 것 (sequentialthinking MCP 도구 활용)
10. 시각화를 적극 활용할 것 — 표, 순서도, 시퀀스 다이어그램, ASCII 차트 등으로 복잡한 개념을 시각적으로 표현
11. 이모티콘을 적절히 사용하여 가독성을 높일 것
12. 답변을 구조화할 것 — 헤더, 리스트, 테이블 등을 활용하여 정보 계층을 명확히
13. 약자 사용 시 반드시 괄호 안에 풀 네임을 병기할 것 (예: CB (Circuit Breaker))
...
- 배포: 파일 생성 후 `just link`를 실행하여 stow로 전역 심링크를 배포
...
- 플랜·아이디어·답변의 기술적 사실 여부·실현 가능성·타당성을 다관점 수렴 검증해야 할 때 → /rl-verify 스킬 사용 (RESEARCHER, CONTRARIAN 등 다관점 subagent 오케스트레이션 + 안정 카운터 기반 수렴 판정)
  - CONTRARIAN(반론·적대적·비관적) 관점이 필요할 때 → compound-engineering:ce-adversarial-document-reviewer 에이전트 사용 (전제 도전, 가정 표면화, 결정 스트레스 테스트, 대안 검토 — 문서의 인식론적 정당성 검증)
...
- **하니스/도구 용어 제거**: `code-review`, `ce-code-review`, `document-review`, `rl-verify`, `persona`, `adversarial` 등 AI 도구·내부 워크플로우 용어를 결과 문서 본문에 노출하지 말 것 → "코드 리뷰를 거쳐 주요 결함 없음" 수준의 사실만.
